...
function ThemeToggle() {
  ...
  return (
    <button
      ...
      className="flex h-11 w-full items-center justify-between rounded-control border border-rule px-3 text-sm text-ink-2 motion-safe-transition hover:border-rule-strong hover:text-ink"
    >
          ...
          return (
            <li key={item.href}>
              <Link
                ...
                className={`flex min-h-11 items-center gap-2 rounded-control border-l-2 px-3 py-2 text-sm motion-safe-transition ${
                  active
  ...
  return (
    <div className="flex h-full flex-col gap-6 p-4">
      <Link href="/" className="block rounded-control">
        ...
        <span className="mt-1 block text-xs text-muted">Mandi decision intelligence</span>
      </Link>
      ...
      <div className="mt-auto flex flex-col gap-4">
        ...
        <details className="text-sm text-ink-2">
          <summary className="min-h-11 cursor-pointer leading-[2.75rem] hover:text-ink">
            Method &amp; assumptions
          </summary>
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh">
      ...
      <aside className="fixed inset-y-0 left-0 z-10 hidden w-60 border-r border-rule bg-paper-2 lg:block">
        <RailContent />
      </aside>
      ...
      <header className="sticky top-0 z-40 flex items-center justify-between border-b border-rule bg-paper-2 px-4 py-3 lg:hidden">
        ...
        <div className="flex items-center gap-3">
          <span className="numeric text-xs text-muted">{SNAPSHOT_LABEL}</span>
          <button
            ref={menuButtonRef}
            type="button"
            ...
            className="min-h-11 rounded-control border border-rule px-3 text-sm text-ink motion-safe-transition hover:border-rule-strong"
          >
            Menu
          </button>
        </div>
      ...
      {sheetOpen ? (
        <div
          ...
          className="fixed inset-x-0 top-[57px] bottom-0 z-modal border-b border-rule-strong bg-paper p-4 lg:hidden"
        >
